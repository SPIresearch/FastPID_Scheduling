...
class ResNetPretrain(nn.Module):
    def __init__(self, block, layers, modality, num_classes=1000, pool='avgpool', zero_init_residual=False,
        ...
        if modality == 'audio':
            ...
            # 使用预训练权重的平均值初始化单通道
            pretrained_weight = pretrained_model.conv1.weight.data
            ...
            pretrained_weight = pretrained_model.conv1.weight.data
            self.conv1.weight.data = pretrained_weight[:, :2, :, :]
        elif modality == 'visual':
            # 直接使用预训练模型的第一层卷积
            self.conv1 = pretrained_model.conv1
        else:
            raise NotImplementedError('Incorrect modality')
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.modality in ['visual', 'flow']:  # 添加 'flow'
            (B, C, T, H, W) = x.size()
            x = x.permute(0, 2, 1, 3, 4).contiguous()
            x = x.view(B * T, C, H, W)
...
def resnet18(modality, pretrain=False, progress=True, **kwargs):
    if pretrain:
        model = ResNetPretrain(BasicBlock, [2, 2, 2, 2], modality, **kwargs)

    else:
        model = _resnet('resnet18', BasicBlock, [2, 2, 2, 2], modality, progress,
                   **kwargs)
    return model